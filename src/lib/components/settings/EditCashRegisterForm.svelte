<script lang="ts">
  ...
  import { updateCashRegisterSchema } from '$lib/settings/schema';
  ...
  let {
    ...
    form: SuperValidated<Infer<typeof updateCashRegisterSchema>>;
    submitError?: string | null;
    submitting?: boolean;
    onSubmit: (data: Infer<typeof updateCashRegisterSchema>) => Promise<void>;
  } = $props();

  const cashRegisterForm = superForm(
    untrack(() => form),
    {
      validators: zod4Client(updateCashRegisterSchema),
...
<form class="space-y-5" onsubmit={handleSubmit}>
  ...
  <div class="flex items-start justify-between gap-4 py-1">
    <div class="space-y-1">
      <Label for="cash-register-enabled">Caja habilitada</Label>
      <p class="text-sm leading-6 text-muted-foreground">
        {$formData.status === 'enabled'
          ? 'La caja está disponible para uso.'
          : 'La caja está deshabilitada.'}
      </p>
    </div>
    <Switch
      id="cash-register-enabled"
      checked={$formData.status === 'enabled'}
      disabled={submitting}
      onCheckedChange={(checked) => {
        $formData.status = checked ? 'enabled' : 'disabled';
      }}
    />
  </div>
